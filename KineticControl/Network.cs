...
namespace KineticControl
{
    public class Network
    {
        ...
        private List<PDS60ca> _pdss = new List<PDS60ca>();
 ...
 */
        ...
        {
            int length = (HexStrings.IntialHex.Length + HexStrings.AddressOff.Length) / 2;
            byte[] colorData = new byte[length];
            byte[] initial;

            if (network == 1)
            {
                initial = HexStrings.IntialHex;
            }
            else
            {
                initial = HexStrings.IntialHex2;
            }
